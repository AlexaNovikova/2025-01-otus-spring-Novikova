...
import ru.otus.inventory.lib.InventoryServiceClient;
...
public class OrderService {
    ...
    @Transactional
    public CartResponseDto addItem(AddItemRequestDto dto, Long userId) {
        Cart cart;
        ...
        try {
            availableQuantity = inventoryServiceClient
                    .getActualQuantity(Long.valueOf(newProductId)).getQuantity();
        ...
        newCartItem.setProductId(Long.valueOf(newProductId));
    ...
    public OrderResponseDto submit(ShopUser shopUser) {
        ...
        order.setEmail(shopUser.getEmail());
        var createdOrder = orderProcessorService.createOrder(order);
        cartRepository.deleteById(userId);
        return orderMapper.map(createdOrder);
    }

    @Transactional
    public OrderResponseDto getStatus(Long orderId, ShopUser shopUser) {
        ...
        if (shopUser.getRoles().contains(Role.CLIENT) && !Objects.equals(order.getUserId(), shopUser.getId())) {
    ...
    @Transactional
    public OrderResponseDto setStatus(Long orderId, OrderStatus status) {
